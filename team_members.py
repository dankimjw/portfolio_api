...
def team_members_get_edit_delete(team_member_id):
    ...
    if request.method == 'GET':
        ...
        if team_member_entity is None:
            ...
            res.status_code = 200
            return res
    # Delete a team_member with team_member_id
        ...
        if team_member_entity is None:
            ...
            if team_member_entity["projects"] is not None:
                ...
                if project_entity is not None:
                    # Remove the team_member_id from the team_members attribute in the project entity
                    project_entity["team_members"].remove({'id': team_member_entity.key.id})
    ...
    # PATCH method: only edit some attributes of a team_members with team_member_id
        ...
        if team_member_entity is None:
            ...
            for key in content.keys():
                # Delete the current team_member-projects relationship if it exists
                if key == "projects" and team_member_entity["projects"] is not None:
                    ...
                    if old_project_entity is not None:
                        # Set the old project's team_member attribute to None or Null
                ...
                # Add the new team_member-projects relationship if it exists
                if key == "projects" and content["projects"] is not None:
                    ...
                    if new_project_entity is not None:
                        # Set the team_members attribute to the id of the current team_member entity
                ...
                # Update the team_member entity
                team_member_entity[key] = content[key]
            # Save the updated team_member entity in DataStore and generate a self link
